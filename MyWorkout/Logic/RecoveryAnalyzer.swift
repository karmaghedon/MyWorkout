...
struct RecoveryAnalyzer {
    ...
    static func warnings(
        ...
        warnings.append(
            contentsOf: volumeSpikeWarnings(
                logs: logs,
                registry: registry
            )
        )
        ...
        warnings.append(
            contentsOf: performanceDeclineWarnings(
                logs: logs
            )
        )

        warnings.append(
            contentsOf: repeatedIntraWorkoutFatigueWarnings(
                logs: logs
            )
        )
    ...
    // MARK: - Intra-Workout Fatigue

    private static func repeatedIntraWorkoutFatigueWarnings(
        logs: [WorkoutLog]
    ) -> [RecoveryWarning] {
    ...
    private static func volumeSpikeWarnings(
        logs: [WorkoutLog],
        registry: ExerciseRegistry
    ) -> [RecoveryWarning] {
    ...
    // MARK: - Performance Decline

    private static func performanceDeclineWarnings(
        logs: [WorkoutLog]
    ) -> [RecoveryWarning] {
        let grouped = groupPerformancesByExercise(
            logs: logs
        ...
        return grouped.compactMap { _, performances in
            ...
            let declinedFromPrevious =
                isWorse(
                    newest,
                    than: previous
                )

            let previousDeclinedFromOldest =
                isWorse(
                    previous,
                    than: oldest
                )

            guard declinedFromPrevious,
                  previousDeclinedFromOldest else {
                return nil
            }
