...
struct UserSettings: Codable {
    ...
    func displayWeight(_ pounds: Int) -> Int {
        switch unitSystem {
        case .pounds:
            return pounds
        case .kilograms:
            return Int((Double(pounds) * 0.453592).rounded())
        }
    }

    func storageWeight(fromDisplayed value: Int) -> Int {
        switch unitSystem {
        case .pounds:
            return value
        case .kilograms:
            return Int((Double(value) / 0.453592).rounded())
        }
    }
}
